...
**프론트엔드** — 인증은 `X-User-Id` 헤더, 채팅은 `GET /collabs/{id}/messages` 3~5초 폴링.
채팅방 진입 시 `GET /collabs/{id}/digest`를 한 번 호출하면 자동 생성 로직이 서버에서 처리된다.
다이제스트 항목의 `source_ids`는 원문 메시지 점프 링크로 렌더링한다.
...
```bash
curl -X POST /demo/seed                      # 한↔미 시나리오 생성, 5단계 가이드 반환
curl -X POST /demo/time -d '{"user_ids": ["<kr>", "<us>"], "now": "2026-08-20T11:00:00Z"}'
...
- `/demo/time`, `/demo/seed`는 **인증 없는 시연 전용 백도어**다. 계정의 '현재 시각'을 바꾸고
