...
배포 서버에서는 두 호출 모두 `X-Demo-Key` 헤더가 필요하다(로컬은 불필요).
...
- **인증**: 모든 보호 엔드포인트는 헤더 `X-User-Id: <id>` 를 요구한다 (해커톤 간이 방식, 추후 JWT 교체).
  `POST /api/v1/auth/signup`, `POST /api/v1/auth/login`은 무인증이다.
  데모 API는 `X-User-Id` 대신 `X-Demo-Key`를 쓴다(④ 참조).
...
**인증** — 서버에 환경변수 `DEMO_KEY`가 설정돼 있으면 이 섹션의 모든 엔드포인트가
헤더 `X-Demo-Key: <값>` 일치를 요구한다(불일치·부재 시 `403`).
설정돼 있지 않으면 무인증이다 — 로컬 개발·테스트는 그대로 쓰면 된다.
**배포 서버에는 반드시 설정하며, 프론트의 '시간 경과'·'접속' 버튼도 이 헤더를 포함해야 한다.**
...
> 데모 라우터는 해커톤 기간 한정으로 배포본에 포함한다(심사 시연에 필요).
> 그동안의 최소 보호가 위 `DEMO_KEY`이며, 해커톤 종료 후에는 라우터 등록 자체를 제거한다.
