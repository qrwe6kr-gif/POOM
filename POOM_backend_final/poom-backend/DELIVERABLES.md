...
API 계약 문서는 **docs/api_spec_v2.md**, DB 스키마는 **docs/schema_v2.sql** 하나씩으로
일원화되었다(구 api_spec.md·schema.sql은 폐기, 기준본 안내문만 남아 있다).
...
| **DB 스키마 v2** | docs/schema_v2.sql | **기준본(canonical)** — app/models.py와 컬럼 단위로 1:1 일치 |
| 크레딧 시스템 설명서 | docs/POOM_credit_brief.pdf | 팀 공유·심사 질의 대응용 |
| ~~구현 기준 명세~~ | api_spec.md | **폐기** — v2로 일원화. 기준본을 가리키는 안내문만 남아 있다 |
| ~~구현 기준 스키마~~ | schema.sql | **폐기** — v2로 일원화. 기준본을 가리키는 안내문만 남아 있다 |
...
주: **외부 계약(경로·요청/응답 JSON 키·상태 값)과 내부 모델·DB 컬럼이 모두 v2와 일치한다.**
남은 표현 차이는 `app/contract.py` 경계 한 곳에서만 변환한다 — 상태 엔진의 소문자 state,
조회자 언어 라벨, 12시간제 시각, 다이제스트 payload 키, 그리고 계약 키 role/body ↔
DB 컬럼 skill/content 네 쌍이다.
